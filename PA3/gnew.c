...
int main(int argc, char* argv[]){
  ...
  inputread(k_len, inputFilename, &newhead, optionalFilename); //reads the inputs from the file
...
void inputread(int len, char * file, GNode ** head, char * optname)
{
	char * in = NULL;
	long size = 0;
	int * arr = NULL;
  ...
  FILE * fp = fopen(optname, "w");

	while( fread(&in, sizeof(char)*len,1,fptr) != EOF)
	{
    ...
    if(in[0] =='\0' | in[len-1] == '\0')
    {
      arr[value] = 1;
		...
		if(arr[value] == 0)
		{
      fwrite(in, sizeof(char)*len,1,fp);
			/*if(head == NULL)
			{
